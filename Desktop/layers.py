...
class FullyConnected(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        Z = np.add(X @ self.parameters['W'], self.parameters['b'])
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        c = int((in_cols + 2*self.pad[1] - kernel_width) / self.stride + 1)
        r = int((in_rows + 2*self.pad[0] - kernel_height) / self.stride + 1)
        X2 = np.pad(X, mode='constant', pad_width=((0, 0), (self.pad[0],
            self.pad[0]), (self.pad[1], self.pad[1]), (0, 0)))
        Z = np.empty((n_examples, r, c, out_channels))
        for row in range(r):
            for column in range(c):
                for color in range(out_channels):
                    Z[:, row, column, color] = np.sum(X2[:,row*self.stride : row*self.stride + kernel_height, 
                                                         column*self.stride : column*self.stride + kernel_width, :] 
                                                      * W[:, :, :, color], axis=(1, 2, 3)) + b[:,color]
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        kernel_height, kernel_width, in_channels, out_channels = W.shape
        n_examples, in_rows, in_cols, in_channels2 = X.shape
        kernel_shape = (kernel_height, kernel_width)
        c = int((in_cols + 2*self.pad[1] - kernel_width) / self.stride + 1)
        r = int((in_rows + 2*self.pad[0] - kernel_height) / self.stride + 1)
        X2 = np.pad(X, mode='constant', pad_width=((0, 0), (self.pad[0],
        ...
        for row in range(r):
            for column in range(c):
                for color in range(out_channels):
                    dX2[:,row*self.stride : row*self.stride + kernel_height, column*self.stride : 
                        column*self.stride + kernel_width,:] += W[np.newaxis, :, :, :, color] * dZ[:, 
                        row:row+1, column:column+1, np.newaxis, color] 
                    dW[:, :, :, color] += np.sum(X2[:, row*self.stride : row*self.stride + kernel_height, 
                                                    column*self.stride : column*self.stride + 
                                                    kernel_width,:] * dZ[:, row:row+1, column:column+1, 
                                                                         np.newaxis, color], axis=0)
        self.gradients["W"] = dW
        self.gradients["b"] = db
        dX = dX2[:, self.pad[0] : in_rows+self.pad[0], self.pad[1] : in_cols+self.pad[1], :]
        ### END YOUR CODE ###
...
class Pool2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        X2 = np.pad(X, mode='constant', pad_width=((0, 0), (self.pad[0],
            self.pad[0]), (self.pad[1], self.pad[1]), (0, 0)))
        X_pool = np.zeros((n_examples, r, c, in_channels))
        # implement the forward pass
        for row in range(r):
            for column in range(c):
                X_pool[:, row, column, :] = self.pool_fn(X2[:, row*self.stride : row * self.stride 
                                                            + kernel_height, column*self.stride 
                                                            : column * self.stride + kernel_width, :], axis=(1, 2))
        # cache any values required for backprop
        self.cache['X'] = X
        ### END YOUR CODE ###
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        c = int((in_cols + 2*self.pad[1] - kernel_width) / self.stride + 1)
        r = int((in_rows + 2*self.pad[0] - kernel_height) / self.stride + 1)
        X2 = np.pad(X, mode='constant', pad_width=((0, 0), (self.pad[0], self.pad[0]), (self.pad[1], self.pad[1]), (0, 0)))
        dX = np.zeros_like(X2)
        # perform a backward pass
        for n in range(n_examples):
            for row in range(r):
                for column in range(c):
                    for color in range(in_channels2):
                        if self.mode == "average":
                            dX[n, row * self.stride : row * self.stride + kernel_height, 
                               column * self.stride : column * self.stride + kernel_width,
                               color] += self.pool_fn(dLdY[n, row, column, color])
                        elif self.mode == "max":
                            temp = X2[n, row * self.stride : row * self.stride + kernel_height,
                                      column * self.stride : column * self.stride +
                                      kernel_width,color]
                            local = np.zeros_like(temp).astype(bool)
                            a, b = np.argwhere(temp == self.pool_fn(temp))[0]
                            local[a, b] = True
                            dX[n, row * self.stride : row * self.stride + kernel_height,
                               column * self.stride : column * self.stride + kernel_width,
                               color] += (local * dLdY[n, row, column, color])
        ### END YOUR CODE ###
