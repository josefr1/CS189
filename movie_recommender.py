...
def update_movie_vecs(user_vecs, movie_vecs, R, movie_rated_idxs):
    ...
    for i in range(R.shape[1]):
        ...
        for j in movie_rated_idxs[i]:
            X += np.outer(user_vecs[j], user_vecs[j])
            Y += R[j, i] * user_vecs[j]
        movie_vecs[i] = np.linalg.inv(X) @ Y
    return movie_vecs
